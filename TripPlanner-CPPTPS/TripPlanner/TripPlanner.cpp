...
void displayCurrentTrip() {
	if (first_time) {
		wcout << L"Trip Stops:\n";
		wcout << L"Trip Legs:\n";
		wcout << L"Total Trip Distance: 0 Miles\n";
		first_time = false; 
	}
	else {
		...
		*/
		for (size_t i = 0; i < stops.size() - 1; ++i) {
			std::wstring startt = stops[i];
			std::wstring endd = stops[i + 1];
			wcout << L"Finding trip stops from " << startt << L" to " << endd << L":\n";
		}
		wcout << L"Trip Stops:\n";
		for (size_t x = 0; x < stops.size(); x++) {
			wcout << L"  " << (x + 1) << L". " << stops[x] << endl;
		}
		...
		/*
			...
			consecutively.
		*/
		float total_total_Distance = 0.0f;
		wcout << L"Trip Legs:\n";
		for (size_t i = 0; i < stops.size() - 1; ++i) {
			wcout <<(i + 1) << L". ";
			vector<wstring> path;
			std::wstring start = stops[i];
			std::wstring end = stops[i + 1];
			...
			for (size_t j = 0; j < path.size(); ++j) {
				...
				if (j < path.size() - 1) {
					wcout << L"-";
					Airport* a1 = graph.getNodeData(path[j]);
					Airport* a2 = graph.getNodeData(path[j + 1]);
					total_distance += Airport::calculateDistance(a1, a2);
				}
			}
			wcout << L" (" << total_distance << L" Miles)\n\n";
		...
		*/
		...
		wcout << L"Total Trip Distance: " << total_total_Distance << L" Miles\n\n";
	}
}
...
bool processUserInput() {
	...
	if (entry.compare(L"S") == 0) {
		...
		vector<wstring> keys; 
		graph.getKeys(&keys); 
		if (find(keys.begin(), keys.end(), entry) != keys.end()) {
			addStop_transaction* transaction = new addStop_transaction(&stops, entry);
			tps.addTransaction(transaction);
		}
		else {
			wcout << L"Invalid airport. Please try again." << endl;
		}
	}
	// UNDO A TRANSACTION
